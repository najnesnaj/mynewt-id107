...
static int si1143_shell_cmd(int argc, char **argv);

static struct shell_cmd si1143_shell_cmd_struct = {
    .sc_cmd = "si1143",
    .sc_cmd_func = si1143_shell_cmd
};
...
static int
si1143_shell_err_too_many_args(char *cmd_name)
{
...
static int
si1143_shell_err_unknown_arg(char *cmd_name)
{
...
static int
si1143_shell_err_invalid_arg(char *cmd_name)
{
...
static int
si1143_shell_help(void)
{
    console_printf("%s cmd [flags...]\n", si1143_shell_cmd_struct.sc_cmd);
    console_printf("cmd:\n");
...
static int
si1143_shell_cmd_read(int argc, char **argv)
{
    uint16_t full;
    uint16_t ir;
    uint16_t samples = 1;
    ...
    if (argc > 3) {
        return si1143_shell_err_too_many_args(argv[1]);
    }
    ...
    if (argc == 3) {
        ...
        if (rc) {
            return si1143_shell_err_invalid_arg(argv[2]);
        }
        samples = val;
    }
    ...
    while(samples--) {
        ...
        rc = si1143_get_data(&g_sensor_itf, &full, &ir);
        if (rc) {
...
static int
si1143_shell_cmd_gain(int argc, char **argv)
{
    uint8_t val;
    uint8_t gain;
    int rc;
    ...
    /* Display the gain */
    if (argc == 2) {
        rc = si1143_get_gain(&g_sensor_itf, &gain);
        if (rc) {
    ...
    if (argc == 3) {
        ...
        rc = si1143_set_gain(&g_sensor_itf, val ?
...
static int
si1143_shell_cmd_time(int argc, char **argv)
{
    uint8_t time;
    long val;
    int rc;
    ...
    if (argc == 2) {
        rc = si1143_get_integration_time(&g_sensor_itf, &time);
        if (rc) {
    ...
    if (argc == 3) {
        ...
        switch(val) {
            case 13:
                rc = si1143_set_integration_time(&g_sensor_itf, TSL2561_LIGHT_ITIME_13MS);
            break;
            case 101:
                rc = si1143_set_integration_time(&g_sensor_itf, TSL2561_LIGHT_ITIME_101MS);
            break;
            case 402:
                rc = si1143_set_integration_time(&g_sensor_itf, TSL2561_LIGHT_ITIME_402MS);
            break;
        }
...
static int
si1143_shell_cmd_int(int argc, char **argv)
{
    int rc;
    int pin;
    uint16_t val;
    ...
    if (argc == 3 && strcmp(argv[2], "on") == 0) {
        return si1143_enable_interrupt(&g_sensor_itf, 1);
    }

    /* Disable the interrupt */
    if (argc == 3 && strcmp(argv[2], "off") == 0) {
        return si1143_enable_interrupt(&g_sensor_itf, 0);
    }

    /* Clear the interrupt on 'clr' */
    if (argc == 3 && strcmp(argv[2], "clr") == 0) {
        return si1143_clear_interrupt(&g_sensor_itf);
    }
    ...
    if (argc == 6 && strcmp(argv[2], "set") == 0) {
        ...
        if (rc) {
            return si1143_shell_err_invalid_arg(argv[3]);
        }
        rate = val;
        /* Get lower threshold */
        val = parse_ll_bounds(argv[4], 0, UINT16_MAX, &rc);
        if (rc) {
            return si1143_shell_err_invalid_arg(argv[4]);
        }
        lower = val;
        /* Get upper threshold */
        val = parse_ll_bounds(argv[5], 0, UINT16_MAX, &rc);
        if (rc) {
            return si1143_shell_err_invalid_arg(argv[5]);
        }
        upper = val;
        /* Set the values */
        rc = si1143_setup_interrupt(&g_sensor_itf, rate, lower, upper);
        if (rc) {
...
static int
si1143_shell_cmd_en(int argc, char **argv)
{
    char *endptr;
    long lval;
    int rc;
    ...
    if (argc == 2) {
        rc = si1143_get_enable(&g_sensor_itf, &enabled);
        if (rc) {
    ...
    if (argc == 3) {
        ...
        if (argv[2] != '\0' && *endptr == '\0' &&
            lval >= 0 && lval <= 1) {
            rc = si1143_enable(&g_sensor_itf, lval);
            if (rc) {
...
static int
si1143_shell_cmd_dump(int argc, char **argv)
{
    uint8_t val;
    int rc;
...
static int
si1143_shell_cmd(int argc, char **argv)
{
    if (argc == 1) {
        return si1143_shell_help();
    }

    /* Read command (get a new data sample) */
    if (argc > 1 && strcmp(argv[1], "r") == 0) {
        return si1143_shell_cmd_read(argc, argv);
    }

    /* Gain command */
    if (argc > 1 && strcmp(argv[1], "gain") == 0) {
        return si1143_shell_cmd_gain(argc, argv);
    }

    /* Integration time command */
    if (argc > 1 && strcmp(argv[1], "time") == 0) {
        return si1143_shell_cmd_time(argc, argv);
    }

    /* Enable */
    if (argc > 1 && strcmp(argv[1], "en") == 0) {
        return si1143_shell_cmd_en(argc, argv);
    }

    /* Interrupt */
    if (argc > 1 && strcmp(argv[1], "int") == 0) {
        return si1143_shell_cmd_int(argc, argv);
    }

    /* Debug */
    if (argc > 1 && strcmp(argv[1], "dump") == 0) {
        return si1143_shell_cmd_dump(argc, argv);
    }

    return si1143_shell_err_unknown_arg(argv[1]);
}

int
si1143_shell_init(void)
{
    int rc;

    rc = shell_cmd_register(&si1143_shell_cmd_struct);
